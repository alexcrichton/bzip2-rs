...
impl<R: Read> Read for BzCompressor<R> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        if self.pos == self.buf.len() {
            self.buf.truncate(0);
            try!(self.r.by_ref().take(self.buf.capacity() as u64)
                       .read_to_end(&mut self.buf));
            eof = self.buf.len() < self.buf.capacity();
            self.pos = 0;
        }
        ...
        let rc = self.stream.compress(&self.buf[self.pos..], buf, action);
...
impl<R: Read> Read for BzDecompressor<R> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let before_in = self.stream.total_in();
        let before_out = self.stream.total_out();
        let rc = self.stream.decompress(&self.buf[self.pos..], buf);
        self.pos += (self.stream.total_in() - before_in) as usize;
        let read = (self.stream.total_out() - before_out) as usize;
